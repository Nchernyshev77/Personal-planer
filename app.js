...
function startReorderDrag(e, id, node){
  e.preventDefault();
  e.stopPropagation();
  ...
  // Keep place in list with placeholder
  ...
  dragState = { id, node, list, placeholder, ghost, offsetY };

  const move = (ev) => onReorderMove(ev);
  const up = () => endReorderDrag(move);
...
function onReorderMove(e){
  if (!dragState) return;
  const { list, placeholder, ghost, offsetY, node } = dragState;

  ghost.style.top = (e.clientY - offsetY) + "px";

  const items = [...list.querySelectorAll(".task")].filter(el => el !== node && el.style.display !== "none");
  let inserted = false;
...
function endReorderDrag(moveHandler){
  if (!dragState) return;
  const { list, placeholder, ghost, node } = dragState;
  ...
  // Place node at placeholder
  placeholder.replaceWith(node);

  // Build task order from DOM
  const ids = [...list.querySelectorAll(".task")].map(el => el.dataset.id).filter(Boolean);
  const byId = new Map(state.tasks.map(t => [t.id, t]));
  state.tasks = ids.map(id => byId.get(id)).filter(Boolean);
...
function render(){
  ...
  for (const t of tasks){
    ...
    // Reorder drag (handle)
    $(".drag", node).addEventListener("pointerdown", (e) => startReorderDrag(e, t.id, node));
