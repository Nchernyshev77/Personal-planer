// Persanal Planer — задачи (перетаскивание, метки, время, автосейв в файл).
// Сохранение:
...
function serializeTasks(){
  return JSON.stringify({ version: 2, updatedAt: nowISO(), tasks: state.tasks }, null, 2);
}
...
function updateText(id, text){
  ...
  t.text = v;
  t.updatedAt = nowISO();
  // no full rerender, keep caret stable
  debouncedSave();
}
...
function renderTaskById(id){
  ...
  node.classList.toggle("done", !!t.done);
  node.classList.remove("tag-none","tag-blue","tag-yellow","tag-red","tag-purple");
  node.classList.add("tag-" + (t.tag || "none"));
...
/* ---------- Pointer drag reorder + animation (FLIP) ---------- */
...
function findInsertBefore(container, y, placeholder){
...
function pointerMove(e){
  ...
  const list = $("#tasks");
  const first = measurePositions();
  const beforeEl = findInsertBefore(list, e.clientY, dragState.placeholder);
